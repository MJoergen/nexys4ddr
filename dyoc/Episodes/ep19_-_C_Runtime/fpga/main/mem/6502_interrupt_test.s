...
.macro trap_eq
        beq *           ;failed .setal (zero)
        .endmacro

.macro trap_ne
        bne *           ;failed not .setal (non zero)
        .endmacro
...
.macro trap_ne
      .local skip
        beq skip
        trap            ;failed not .setal (non zero)
skip
        .endmacro
; please observe that during the test the stack gets invalidated
; therefore a RTS inside the success .macro is not possible
...
.macro push_stat arg1       ;setting flags in the processor status register
            lda #arg1
            pha         ;use stack to load status
            .endmacro

.macro set_stat arg1      ;setting flags in the processor status register
            lda #arg1
            pha         ;use stack to load status
            plp
            .endmacro
...
lst_f   .set $1fe            ;last flags before interrupt
lst_a   .set $1ff            ;last accumulator before interrupt
