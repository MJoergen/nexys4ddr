...
carry   .set %00000001   ;flag bits in status
zero    .set %00000010
intdis  .set %00000100
decmode .set %00001000
break   .set %00010000
reserv  .set %00100000
overfl  .set %01000000
minus   .set %10000000
...
fnc     .set minus+carry
fnz     .set minus+zero
fnzc    .set minus+zero+carry
fnv     .set minus+overfl

fao     .set break+reserv    ;bits always on after PHP, BRK
fai     .set fao+intdis      ;+ forced interrupt disable
faod    .set fao+decmode     ;+ ignore decimal
faid    .set fai+decmode     ;+ ignore decimal
m8      .set $ff             ;8 bit mask
m8i     .set $ff&~intdis     ;8 bit mask - interrupt disable
...
;masking of always on bits after PHP or BRK (unused & break) on compare
    .if disable_decimal < 2
        .if I_flag = 0
.macro load_flag  param_1
            lda #(param_1&m8i)         ;force enable interrupts (mask I)
            .endmacro
.macro cmp_flag   param_1
            cmp #((param_1|fao)&m8i)   ;I_flag is always enabled + always on bits
            .endmacro
.macro eor_flag   param_1
            eor #(param_1&m8i|fao)   ;mask I, invert expected flags + always on bits
            .endmacro
        .endif
        .if I_flag = 1
.macro load_flag  param_1
            lda #(param_1|intdis)      ;force disable interrupts
            .endmacro
.macro cmp_flag   param_1
            cmp #((param_1|fai)&m8)    ;I_flag is always disabled + always on bits
            .endmacro
.macro eor_flag   param_1
            eor #(param_1|fai)       ;invert expected flags + always on bits + I
            .endmacro
        .endif
        .if I_flag = 2
.macro load_flag  param_1
...
.macro cmp_flag   param_1
            ...
            cmp #((param_1|fao)&m8i)   ;expected flags + always on bits, mask I
            .endmacro
.macro eor_flag   param_1
            eor flag_I_on       ;I_flag is never changed
            eor #(param_1&m8i|fao)   ;mask I, invert expected flags + always on bits
        ...
        .if I_flag = 3
.macro load_flag  param_1
            lda #param_1             ;allow test to change I-flag (no mask)
            .endmacro
.macro cmp_flag   param_1
            cmp #((param_1|fao)&m8 )   ;expected flags + always on bits
            .endmacro
.macro eor_flag   param_1
            eor #((param_1)|fao )        ;invert expected flags + always on bits
            .endmacro
        .endif
    .else
        .if I_flag = 0
...
.macro cmp_flag   param_1
            ...
            cmp #((param_1|faod)&m8i)  ;I_flag is always enabled + always on bits
            .endmacro
.macro eor_flag   param_1
            ora #decmode        ;ignore decimal mode bit
            eor #((param_1&m8i|faod))  ;mask I, invert expected flags + always on bits
            .endmacro
        .endif
        .if I_flag = 1
.macro load_flag  param_1
...
.macro cmp_flag   param_1
            ora #decmode        ;ignore decimal mode bit
            cmp #((param_1|faid)&m8)   ;I_flag is always disabled + always on bits
            .endmacro
.macro eor_flag   param_1
            ora #decmode        ;ignore decimal mode bit
            eor #(param_1|faid)      ;invert expected flags + always on bits + I
            .endmacro
        .endif
        .if I_flag = 2
.macro load_flag  param_1
...
.macro cmp_flag   param_1
            eor flag_I_on       ;I_flag is never changed
            ora #decmode        ;ignore decimal mode bit
            cmp #((param_1|faod)&m8i)  ;expected flags + always on bits, mask I
            .endmacro
.macro eor_flag   param_1
            eor flag_I_on       ;I_flag is never changed
            ora #decmode        ;ignore decimal mode bit
...
.macro cmp_flag   param_1
            ora #decmode        ;ignore decimal mode bit
            cmp #((param_1|faod)&m8)   ;expected flags + always on bits
            .endmacro
.macro eor_flag   param_1
            ora #decmode        ;ignore decimal mode bit
            eor #(param_1|faod)        ;invert expected flags + always on bits
            .endmacro
        .endif
    .endif

;macros to set (register|memory|zeropage) & status
.macro set_stat  param_1        ;setting flags in the processor status register
            load_flag param_1
            pha         ;use stack to load status
            plp
            .endmacro

.macro set_a   param_1, param_2          ;precharging accu & status
            load_flag param_2
            pha         ;use stack to load status
            lda #param_1     ;precharge accu
            plp
            .endmacro

.macro set_x  param_1, param_2           ;precharging index & status
            load_flag param_2
            pha         ;use stack to load status
            ldx #param_1     ;precharge index x
            plp
            .endmacro

.macro set_y   param_1, param_2          ;precharging index & status
            load_flag param_2
            pha         ;use stack to load status
            ldy #param_1     ;precharge index y
            plp
            .endmacro

.macro set_ax   param_1, param_2         ;precharging indexed accu & immediate status
            load_flag param_2
            pha         ;use stack to load status
            lda param_1,x    ;precharge accu
            plp
            .endmacro

.macro set_ay   param_1, param_2         ;precharging indexed accu & immediate status
            load_flag param_2
            pha         ;use stack to load status
            lda param_1,y    ;precharge accu
            plp
            .endmacro

.macro set_z  param_1, param_2           ;precharging indexed zp & immediate status
            load_flag param_2
            pha         ;use stack to load status
            lda param_1,x    ;load to zeropage
            sta zpt
            plp
            .endmacro

.macro set_zx  param_1, param_2          ;precharging zp,x & immediate status
            load_flag param_2
            pha         ;use stack to load status
            lda param_1,x    ;load to indexed zeropage
            sta zpt,x
            plp
            .endmacro

.macro set_abs  param_1, param_2         ;precharging indexed memory & immediate status
            load_flag param_2
            pha         ;use stack to load status
            lda param_1,x    ;load to memory
            sta abst
            plp
            .endmacro

.macro set_absx   param_1, param_2       ;precharging abs,x & immediate status
            load_flag param_2
            pha         ;use stack to load status
            lda param_1,x    ;load to indexed memory
            sta abst,x
            plp
            .endmacro

;macros to test (register|memory|zeropage) & status & (mask)
.macro tst_stat  param_1          ;testing flags in the processor status register
            php         ;save status
...
.macro tst_a   param_1, param_2          ;testing result in accu & flags
            php         ;save flags
            cmp #param_1     ;test result
            trap_ne
            pla         ;load status
            pha
            cmp_flag param_2
...
.macro tst_x    param_1, param_2         ;testing result in x index & flags
            php         ;save flags
            cpx #param_1     ;test result
            trap_ne
            pla         ;load status
            pha
            cmp_flag param_2
...
.macro tst_y  param_1, param_2           ;testing result in y index & flags
            php         ;save flags
            cpy #param_1     ;test result
            trap_ne
            pla         ;load status
            pha
            cmp_flag param_2
...
.macro tst_ax   param_1, param_2, param_3         ;indexed testing result in accu & flags
            php         ;save flags
            cmp param_1,x    ;test result
            trap_ne
            pla         ;load status
            eor_flag param_3
            cmp param_2,x    ;test flags
            trap_ne     ;
            .endmacro

.macro tst_ay param_1, param_2, param_3           ;indexed testing result in accu & flags
            php         ;save flags
            cmp param_1,y    ;test result
            trap_ne     ;
            pla         ;load status
            eor_flag param_3
            cmp param_2,y    ;test flags
            trap_ne
            .endmacro
        
.macro tst_z   param_1, param_2, param_3          ;indexed testing result in zp & flags
            php         ;save flags
            lda zpt
            cmp param_1,x    ;test result
            trap_ne
...
.macro tst_zx  param_1, param_2, param_3          ;testing result in zp,x & flags
            php         ;save flags
            lda zpt,x
            cmp param_1,x    ;test result
            trap_ne
...
.macro tst_abs  param_1, param_2, param_3         ;indexed testing result in memory & flags
            php         ;save flags
            lda abst
            cmp param_1,x    ;test result
            trap_ne
...
.macro tst_absx   param_1, param_2, param_3       ;testing result in abs,x & flags
            php         ;save flags
            lda abst,x
            cmp param_1,x    ;test result
            trap_ne
...
irq_a:   .byt  1               ;a register
irq_x:   .byt  1               ;x register
    .if I_flag = 2
;masking for I bit in status
flag_I_on:   .byt  1           ;or mask to load flags   
flag_I_off:  .byt  1           ;and mask to load flags
    .endif
zpt  :                       ;5 bytes store/modify test area
;add/subtract operand generation and result/flag prediction
adfc  :  .byt  1               ;carry flag before op
ad1    : .byt  1               ;operand 1 - accumulator
ad2     :.byt  1               ;operand 2 - memory / immediate
adrl   : .byt  1               ;expected result bits 0-7
adrh   :.byt  1               ;expected result bit 8 (carry)
adrf   :.byt  1               ;expected flags NV0000ZC (only binary mode)
sb2    :.byt  1               ;operand 2 complemented for subtract
zp_bss:
...
ind1 :   .addr  abs1            ;indirect pointer to pattern in absolute memory
        .addr  abs1+1
        .addr  abs1+2
        .addr  abs1+3
        .addr  abs7f
inw1  :  .addr  abs1-$f8        ;indirect pointer for wrap-test pattern
indt   : .addr  abst            ;indirect pointer to store area in absolute memory
        .addr  abst+1
        .addr  abst+2
        .addr  abst+3
inwt   : .addr  abst-$f8        ;indirect pointer for wrap-test store
indAN  : .addr  absAN           ;indirect pointer to AND pattern in absolute memory
        .addr  absAN+1
        .addr  absAN+2
        .addr  absAN+3
indEO :  .addr  absEO           ;indirect pointer to EOR pattern in absolute memory
        .addr  absEO+1
        .addr  absEO+2
        .addr  absEO+3
indOR  : .addr  absOR           ;indirect pointer to OR pattern in absolute memory
        .addr  absOR+1
        .addr  absOR+2
        .addr  absOR+3
;add/subtract indirect pointers
adi2  :  .addr  ada2            ;indirect pointer to operand 2 in absolute memory
sbi2  :  .addr  sba2            ;indirect pointer to complemented operand 2 (SBC)
adiy2 :  .addr  ada2-$ff        ;with offset for indirect indexed
sbiy2 :  .addr  sba2-$ff
zp_bss_end:
...
rROL  :  .byt  $86,$04,$82,0   ; "
rROLc :  .byt  $87,$05,$83,1   ;expected result ROL +carry
rLSR  :                      ;expected result LSR & ROR -carry
rROR  :  .byt  $61,$41,$20,0   ; "
rRORc :  .byt  $e1,$c1,$a0,$80 ;expected result ROR +carry
fASL  :                      ;expected flags for shifts
fROL  :  .byt  fnc,fc,fn,fz    ;no carry in
fROLc :  .byt  fnc,fc,fn,0     ;carry in
fLSR:
fROR  :  .byt  fc,0,fc,fz      ;no carry in
fRORc  : .byt  fnc,fn,fnc,fn   ;carry in
...
; target for the jump indirect test
        .align 2
ptr_tst_ind :.addr test_ind
ptr_ind_ret :.addr ind_ret
