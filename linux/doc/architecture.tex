% Block diagram of the proposed Linux-on-Artix-7 architecture.
...
\begin{tikzpicture}[
  ...
  \node[ext, text width=5.0cm] (lan) at (\xE,2.0)
    {\textbf{Network}\\boot server: DHCP, TFTP, signed bundles\\log server: NFS / syslog};
  ...
  \node[ext] (mem)   at (\xE,-4.6)  {Memory module (connector)\\HyperRAM $2\times64$\,MB\\+ module-ID EEPROM};
  \node[ext] (dev)   at (\xE,-6.2)  {On-board sensors\\UART / SPI / I$^2$C};
  \node[ext] (osc)   at (\xE,-7.6)  {Oscillator};
  \node[ext] (jtag)  at (\xE,-10.6) {USB-JTAG\\(development)};
  \node[ext, opt] (gtpc) at (\xE,-11.9) {Connector / SFP\\(optional)};

  % ---------------- Shell (static) ----------------
  \node[io] (io1) at (\xIO,0)    {I/O};
  \node[io] (io2) at (\xIO,-4.6) {I/O};
  \node[io] (io3) at (\xIO,-6.2) {I/O};
  \node[io] (io4) at (\xIO,-7.6) {I/O};
  ...
  \node[shl] (clk)   at (\xS,-7.6)  {Clocking\\MMCM (DRP exposed)};
  \node[shl, minimum height=0.6cm] (xadc) at (\xS,-8.65) {XADC};
  \node[shl, minimum height=0.6cm] (wdg)  at (\xS,-9.5)  {Watchdog, version regs};
  \node[shl] (bscan) at (\xS,-10.6) {BSCANE2};
  \node[shl, opt] (gtp) at (\xS,-11.9) {GTPE2 $\times4$\\(optional)};

  % ---------------- Role (reconfigurable) ----------------
  \node[rol] (eth)  at (\xP,0)     {Ethernet MAC\\(LiteEth)};
  \node[rol] (memc) at (\xP,-4.6)  {Memory controller\\(HyperRAM)};
  \node[rol] (per)  at (\xP,-6.2)  {UART / SPI / I$^2$C\\controllers};
  \node[rol, minimum height=2.3cm] (csr) at (\xP,-8.55) {Shell access\\(clocks, DRP,\\watchdog)};
  \node[rol] (dbg)  at (\xP,-10.6) {Debug module\\(RISC-V debug)};
  \node[rol, opt] (fut) at (\xP,-11.9) {Future use\\(SGMII, PCIe, \dots)};

  \node[draw, fill=blue!22, minimum width=0.6cm, minimum height=11.4cm]
    (bus) at (\xBus,-5.1) {};
  \node[rotate=90] at (bus.center) {System bus (Wishbone / AXI)};

  \node[rol] (cpu)  at (\xC,-1.5) {\textbf{RV64GC CPU}\\(VexiiRiscv)\\MMU, L1 caches};
  \node[rol] (rom)  at (\xC,-3.6) {Boot ROM\\(LiteX BIOS, BRAM)};
  \node[rol] (irq)  at (\xC,-5.4) {PLIC / CLINT\\timers};
  ...
  \draw[lnk] (mem) -- (io2);
  \draw[lnk] (io2) -- node[note, pos=0.35, above] {generic pins} (memc);
  \draw[lnk] (dev) -- (io3);
  ...
  \begin{scope}[on background layer]
    ...
    \node[anchor=north west, align=left, font=\small] at (role.north west)
      {\textbf{ROLE} (reconfigurable partition)\\[-1pt]\scriptsize downloaded over Ethernet at every boot};
